<template>
  <div v-if="isOpen && !embedded" class="connection-modal-overlay" @click="closeModal">
  ...
  <!-- Embedded version without overlay -->
  <div v-if="isOpen && embedded" class="connection-modal-content embedded">
    <!-- Header -->
    <div class="modal-header">
      <h2 class="modal-title">Create New Connection</h2>
      <button v-if="!embedded" @click="closeModal" class="modal-close-btn">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
        </svg>
      </button>
    ...
    <!-- Footer -->
    <div class="modal-footer">
      <button v-if="!embedded" @click="closeModal" class="btn-secondary">
        Close
      </button>
      <button @click="testConnection" :disabled="isLoading" class="btn-test">
...
const props = defineProps<{
  ...
  connection?: any;
  embedded?: boolean; // Add embedded prop to control overlay
}>();
...
/* Embedded version without modal styling */
.connection-modal-content.embedded {
...
/* Embedded header styling */
.connection-modal-content.embedded .modal-header {
  background: transparent;
...
/* Embedded body styling */
.connection-modal-content.embedded .modal-body {
  padding: 0;
}
...
.connection-modal-content.embedded .modal-footer {
  background: transparent;
